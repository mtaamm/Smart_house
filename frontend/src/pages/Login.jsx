    ...
    try {
      if (isLogin) {
        ...
        if (response.data.status === 'successful') {
          const { auth, own_house, root_owner } = response.data.data;
        ...
        const response = await axios.post('http://localhost:3000/user/sign-up', signUpData);
      ...
      if (err.response) {
        setError(err.response.data.message || 'Đã xảy ra lỗi khi kết nối với server');
      } else if (err.request) {
