...
interface HardwareModule {
  init(): Promise<void>;
  getTempHumi(houseId: string, sensorId: number | string): Promise<{ temp?: number; humi?: number }>;
  getLight(houseId: string, sensorId: number | string): Promise<{ light?: number }>;
  getStatus(houseId: string, deviceId: number | string): Promise<DeviceStatus>;
  controlDevice(houseId: string, deviceId: number | string, type: string, on: boolean, lock?: boolean): Promise<boolean>;
  setRgbColor(houseId: string, deviceId: number | string, r: number, g: number, b: number): Promise<boolean>;
  listDevices(): { [houseId: string]: { [deviceId: string]: DeviceStatus & { lastUpdate: number } } };
  listSensors(): { [houseId: string]: { [sensorId: string]: { temp?: number; humi?: number; light?: number; lastUpdate: number } } };
}
...
const hardware = (() => {
  ...
  const formatTopic = (topicTemplate: string, params: { [key: string]: string | number }): string => {
  ...
  const handleIncomingMessage = (topic: string, message: Buffer): void => {
    ...
    if (category === 'sensor') {
      ...
      if (!deviceCache[houseId][deviceId]) {
        deviceCache[houseId][deviceId] = { type: '', on: false, timestamp: Date.now() };
      }
  ...
  const acquireLock = (houseId: string, deviceId: number | string): Promise<void> => {
  ...
  const getTempHumi = async (houseId: string, sensorId: number | string): Promise<{ temp?: number; humi?: number }> => {
    ensureConnection();
    
    if (!sensorCache[houseId] || 
        !sensorCache[houseId][sensorId]) {
  ...
  const getLight = async (houseId: string, sensorId: number | string): Promise<{ light?: number }> => {
    ensureConnection();
  ...
  const getStatus = async (houseId: string, deviceId: number | string): Promise<DeviceStatus> => {
    ensureConnection();
    
    if (!deviceCache[houseId] || 
    ...
    try {
      ...
      switch (type) {
        case 'door':
          ...
          controlTopic = formatTopic(topicFormats.doorControl, { houseId, deviceId });
          break;
        case 'alarm':
          message = on ? 'ON' : 'OFF';
          controlTopic = formatTopic(topicFormats.alarmControl, { houseId, deviceId });
          break;
        case 'fan':
          message = on ? 'ON' : 'OFF';
          controlTopic = formatTopic(topicFormats.fanControl, { houseId, deviceId });
          break;
        case 'rgb':
          message = on ? 'ON' : 'OFF';
          controlTopic = formatTopic(topicFormats.rgbControl, { houseId, deviceId });
          break;
        default:
  ...
  const setRgbColor = async (
    ...
    try {
      const controlTopic = formatTopic(topicFormats.rgbControl, { houseId, deviceId });
